...
if_mask = func.str2bool(args.if_mask)
    ...
    for i, (input, gt, img_path) in enumerate(tbar):
        ...
        for scale in scales:
            ...
            if args.confidence == 2:
                prediction_temp = scale_process(args, model, image_scale, classes, crop_h, crop_w, h, w, mean, std)
                crf_score = imutils.crf_inference(np.ascontiguousarray(image.astype(np.uint8)),np.transpose(prediction_temp,(2,0,1)), labels=classes)
                prediction = prediction + np.transpose(crf_score,(1,2,0))
            else:
        ...
        #  0: no mask for next iteration
        #  1: mask to see
        #  2: for mat
        if if_mask == 0:
            [px,py,pn] = prediction.shape
            cf = np.zeros([px,py,1])
            cf[:,:,:] += args.confidence
            prediction = np.append(prediction,cf, axis=2)
        ...
        if if_mask is 1:
            mask = func.get_mask_pallete ( pred[0].cpu ().numpy (), 'pascal_voc_context' )
        elif if_mask is 2:
            pred=torch.from_numpy(prediction)
            mask = pred.numpy()
            file_name = img_path[0][img_path[0].rfind ( '/' ) + 1 :-4] 
            mask = func.eq2(file_name,mask,args.save_path)
            #np.save ( os.path.join ( args.save_path, file_name ) , mask)                   #file too big!
            continue;
        elif if_mask == 0:
            mask = func.get_confidence_pallete_context ( pred[0].cpu ().numpy (),pn)
...
#for train-seg data
train_dataset = dataset.SemData(split='train', data_root=args.dataset_path, data_list='train_context.txt', 
                                transform=train_transform, path = args.train_path)                            #Remember to change!!
val_dataset = dataset.SemData(split='val', data_root=args.dataset_path, data_list='val_context.txt', 
                                transform=val_transform, path = 'pascal_context_trainval')

train_loader = data.DataLoader(train_dataset, num_workers=args.workers, batch_size=1, shuffle=True, pin_memory=True)
val_loader = data.DataLoader(val_dataset, num_workers=args.workers, batch_size=1, shuffle=False, pin_memory=True)
...
if if_mask is 1:
    ...
    test(args, train_loader, model, args.numclasses, mean, std, 512, 465, 465, [0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0])    #output data for train
