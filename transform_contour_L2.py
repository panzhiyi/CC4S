...
"""
Only some different from transform_contour:
    Add nessery input for Laplace_loss
"""
class Compose(object):
    ...
    def __call__(self, image, label,contour, spiex):
        for t in self.segtransform:
            image, label, contour,spiex = t(image, label, contour,spiex)
        return image, label, contour, spiex
...
class ToTensor(object):
    ...
    def __call__(self, image, label, contour, spiex):
        ...
        if not isinstance(label, torch.LongTensor):
            label = label.long()
        spiex= torch.from_numpy(spiex)
        if not isinstance(spiex, torch.LongTensor):
            spiex = spiex.long()
...
class Resize(object):
    ...
    def __call__(self, image, label, contour, spiex):
        ...
        spiex = cv2.resize(spiex, self.size[::-1], interpolation=cv2.INTER_NEAREST)
        return image, label, contour, spiex
...
class RandScale(object):
    ...
    def __call__(self, image, label, contour, spiex):
        ...
        spiex = cv2.resize(spiex, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_NEAREST)
...
class Crop(object):
    ...
    def __call__(self, image, label, contour, spiex):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            spiex = cv2.copyMakeBorder(spiex, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.ignore_label)
        h, w = label.shape
        ...
        spiex = spiex[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
...
class RandRotate(object):
    ...
    def __call__(self, image, label,contour, spiex):
        if random.random() < self.p:
            ...
            spiex = cv2.warpAffine(spiex, matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=self.ignore_label)
