...
mmconfig_file = 'configs/ocrnet/ocrnet_hr48_512x512_20k_voc12aug.py'
import logging
...
if_laplaceloss = func.str2bool(args.if_laplaceloss)
#print [item for item in args.__dict__.items()]

if if_laplaceloss == 1 or if_laplaceloss == 2:
    import transform_contour_L2 as transform_contour
else:
...
class Trainer():
    ...
    def __init__(self, args):
        ...
        if if_laplaceloss == 1:
            train_dataset = dataset.Sem_ContourData_L2(split='train', data_root=args.dataset_path, data_list=args.data_list, transform=train_transform, path = args.train_path,path_contour= 'superpixels',superpiex2_path = args.spiex_path,Laplace_martix_path = args.laplace_path)
        elif if_laplaceloss == 2:
            train_dataset = dataset.Sem_ContourData_Contrast(split='train', data_root=args.dataset_path, data_list=args.data_list, transform=train_transform, path = args.train_path,path_contour= 'superpixels')
        else:
            train_dataset = dataset.Sem_ContourData(split='train', data_root=args.dataset_path, data_list=args.data_list, transform=train_transform, path = args.train_path,path_contour= 'superpixels')
        ...
        if (args.special_set & 0b11111111) == 0b10000001:
            print('special_set: use context dataset!')
            if if_laplaceloss:
                train_dataset = dataset.Sem_ContourData_L2(split='train', data_root=args.dataset_path, data_list='train_context.txt', transform=train_transform, path = args.train_path,path_contour= 'superpixels_context',superpiex2_path = args.spiex_path,Laplace_martix_path = args.laplace_path)
            else:
                train_dataset = dataset.Sem_ContourData(split='train', data_root=args.dataset_path, data_list='train_context.txt', transform=train_transform, path = args.train_path,path_contour= 'superpixels_context')
            val_dataset = dataset.SemData(split='val', data_root=args.dataset_path, data_list='val_context.txt', transform=val_transform, path = 'pascal_context_trainval')
        ...
        self.model = self.get_swmodel()

        if args.model_path != 'None':
            model_pretrain = torch.load ( args.model_path )
            self.model = func.param_restore_all ( self.model, model_pretrain['state_dict'] )
        else:
            logging.error('Please input the model path!')
            exit()
        ...
        if if_laplaceloss == 1:
            self.laplaceloss = loss.LaplaceLoss_gpu_pzy_ori()
        elif if_laplaceloss == 2:
            self.laplaceloss = loss.My_Contrast_loss()
            
    def get_swmodel(self):
        from mmcv.utils import Config
        import logging
        from mmseg.models import build_segmentor
        cfg = Config.fromfile(mmconfig_file)
        cfg.seed = opt_manualSeed
    ...
    def train(self, epoch):
        ...
        for i, batch in enumerate(tbar):
            ...
            if if_laplaceloss == 1:
                img, gt, edge, path_name, Laplace_W, Laplace_L, spiex = batch
            elif if_laplaceloss == 2:
                img, img2, gt, edge, img_index1, img_index2 = batch
                img_index1 = img_index1.cuda(non_blocking=True)
                img_index2 = img_index2.cuda(non_blocking=True)
            else:
            ...
            pred,x,_,z, P = self.model(img = img_v,return_feat = True)
            # debug()
            
            with torch.no_grad():
                pred_s,x_s,_,z_s, P_s = self.model(img = img_flip_v,return_feat = True)
                P_s.detach_()
            ...
            # cal for Laplace_loss
            lploss = 0
            if if_laplaceloss == 1:
                lploss = self.laplaceloss(pred_sg_up,Laplace_W, Laplace_L, spiex,args.numclasses)
                #print(lploss)
            elif if_laplaceloss == 2:
                if USE_FEATURE_CONTRAST:
                    ###特征层太大,这么做显存不够###
                    z2 = self.model(img2.cuda())[3]
                    random_21_channel = torch.randint(0,z2.shape[-1],(21,))
                    feature2 = F.interpolate(z2.detach()[:,:,:,random_21_channel], size=input_size, mode='bilinear', align_corners=True)
                    feature = F.interpolate(z[:,:,:,random_21_channel], size=input_size, mode='bilinear', align_corners=True)
                    del img2,z2
                    lploss = self.laplaceloss(feature,feature2,img_index1,img_index2)
                    del feature2,img_index1,img_index2
                else:
                    pred2 = self.model(img2.cuda())[0]
                    pred_sg_up2 = F.interpolate(pred2.detach(), size=input_size, mode='bilinear', align_corners=True)
                    del img2,pred2
                    lploss = self.laplaceloss(pred_sg_up,pred_sg_up2,img_index1,img_index2)
                    del pred_sg_up2,img_index1,img_index2

                writer.add_scalar('train/Contrast_loss',lploss.item(),self.iter_count)
                self.iter_count += 1
            ...
            if self.args.edgeloss_weight != 0:
                ...
                loss = loss + self.args.edgeloss_weight * loss_edge+ lploss
